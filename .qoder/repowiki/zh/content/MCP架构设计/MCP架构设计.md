...
该仓库现在采用简化的"协议学习实验室（Next.js）"命名约定，重新组织了模块结构，包含每日学习笔记、教学演示和安全模型演示，用于展示如何通过 MCP 协议与外部工具/资源进行交互。

## 项目结构
整体采用"前端演示 + 独立 MCP 服务"的分层组织方式：
- 前端（Next.js App Router）：提供交互式界面，封装对 MCP 工具的调用能力，并通过自定义 Hook 暴露统一接口。
- 演示服务（Node.js）：实现基于 MCP 的文件系统工具服务器，并提供测试客户端以验证端到端流程。
- 内容模块：重新组织的知识库结构，包含每日学习记录和教学示例。
...
章节来源
- [content/daily/:1-200](file://content/daily/2026-04-23.md#L1-L200)
- [demos/:1-200](file://demos/day1-mcp/package.json#L1-L200)
...
## 结论
本方案通过 MCP 协议将前端与后端工具解耦，利用 Hook 抽象调用细节，使前端专注于交互体验，而服务器专注工具实现与安全控制。经过重构后的知识基础更加简洁明了，支持每日学习记录、教学演示和安全模型演示，具备良好的可扩展性与可维护性，适合在多工具、多环境的场景下演进。
